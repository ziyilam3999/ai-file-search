...
interface AppConfig {
  ...
  embedding: {
    ...
    chunkSize: number;    // 200 words
    chunkOverlap: number; // 50 words
  };
  index: {
...
User: /ask How do I configure the VPN?
  ...
  ├─ 3. Embed query → search hnswlib (top 5)
...
- **Chunking logic**: 200 words, 50 word overlap
...
3. **200-word fact-based answers are still forgiving** — The prompt constrains Monday to report facts only (no reasoning), with strict citation rules. Even a slightly off-topic chunk won't derail a factual answer.
...
| **`n_ctx`** | 2048 | **4096** | More context window = can fit more chunks + longer answer |
| **`top_k` chunks** | 3 | **5** | More source material = better synthesized answer |
| **Chunk size** | 200 words | **200 words** | No change — chunk size is fine |
...
New budget (4096 tokens):
- System prompt: ~120 tokens
- 5 chunks × ~260 tokens = ~1,300 tokens
- Answer: ~300 tokens
- **Total: ~1,720 tokens** (comfortable headroom)

Qwen3.5-2B supports 262K context, so 4096 is trivial for the model. The RAM impact is negligible.

**4. top_k: 3 → 5 chunks**

More source chunks = better synthesized answers. With 50-word answers, 3 chunks was plenty (you could barely reference them all). With 200-word answers, the AI can meaningfully reference 5 sources to give a more complete picture.
...
```yaml
llm:
  contextSize: 4096    # was 2048
  maxTokens: 300       # was 100
  temperature: 0.1     # unchanged
  threads: 8           # unchanged

embedding:
  chunkSize: 200       # unchanged
  chunkOverlap: 50     # unchanged
  topK: 5              # was 3
```
...
async function answerQuestion(query: string, options: { topK?: number } = {}) {
  const topK = options.topK ?? 5;  // was 3
  // ... rest of pipeline
}
```
...
|---------|---------|-----|-------------|
| Retrieval (5 vs 3 chunks) | ~0.1s | ~0.12s | Negligible |
| LLM generation (300 vs 100 tokens) | ~3.3s | ~10s | +6.7s on CPU |
| **Total response time** | **~3.5s** | **~10s** | Acceptable for Slack |
...
```yaml
presets:
  quick:     { maxTokens: 100, topK: 3 }   # ~3s — fast, citation-focused
  standard:  { maxTokens: 300, topK: 5 }   # ~10s — helpful summaries (DEFAULT)
  detailed:  { maxTokens: 500, topK: 7 }   # ~17s — thorough, multi-source
```

Users could eventually choose via `/ask --quick How do I reset my password?` — but default is `standard`.
...
```yaml
presets:
  quick:     { maxTokens: 100, topK: 3 }   # ~1s
  standard:  { maxTokens: 300, topK: 5 }   # ~2.5s  (DEFAULT)
  detailed:  { maxTokens: 500, topK: 7 }   # ~4.2s
```
...
## 20. WhatsApp Integration: Not Now, Design For It Later
