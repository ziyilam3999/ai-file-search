...
TEST_QUERIES = [
    ...
    {"query": "What is the year for the tenancy agreement?", "type": "extraction", "expected_behavior": "answer", "weight": 1.0},
    {"query": "How many credits does a new user receive during onboarding?", "type": "extraction", "expected_behavior": "answer", "weight": 1.0},
    {"query": "How much does the GetSpace Premium subscription cost per month?", "type": "extraction", "expected_behavior": "answer", "weight": 1.0},
    
    # === NUMERIC (Numbers, dates, amounts) ===
    {"query": "What is the maximum number of credits a user can borrow at any time?", "type": "numeric", "expected_behavior": "answer", "weight": 1.5},
    {"query": "How many days do users have to repay borrowed tokens?", "type": "numeric", "expected_behavior": "answer", "weight": 1.5},
    {"query": "What is the penalty amount for failing to repay borrowed tokens on time?", "type": "numeric", "expected_behavior": "answer", "weight": 1.5},
    {"query": "How many vehicles can a user register on their GetSpace account?", "type": "numeric", "expected_behavior": "answer", "weight": 1.5},
    {"query": "How many tokens does a user earn for hosting a parking spot?", "type": "numeric", "expected_behavior": "answer", "weight": 1.5},
    
    # === MULTI-FACT (Multiple pieces of information) ===
    {"query": "What e-wallet platforms are supported for cash payouts?", "type": "multi-fact", "expected_behavior": "answer", "weight": 1.2},
    {"query": "What vehicle sizes does GetSpace support?", "type": "multi-fact", "expected_behavior": "answer", "weight": 1.2},
    {"query": "What are the requirements to become a beta user?", "type": "multi-fact", "expected_behavior": "answer", "weight": 1.2},
    
    # === REASONING (Interpretation, inference) ===
    {"query": "What happens if a seeker's ETA is greater than 40 minutes?", "type": "reasoning", "expected_behavior": "answer", "weight": 2.0},
    {"query": "Can users with unpaid penalties still borrow tokens?", "type": "reasoning", "expected_behavior": "answer", "weight": 2.0},
    {"query": "Is there a penalty for hosts who cancel parking hosting?", "type": "reasoning", "expected_behavior": "answer", "weight": 2.0},
    {"query": "What happens to the deposit token if a seeker accepts a no-show report?", "type": "reasoning", "expected_behavior": "answer", "weight": 2.0},
    
    # === COMPARISON (Compare/contrast) ===
    {"query": "What is the difference between Premium and Freemium users for platform fees?", "type": "comparison", "expected_behavior": "answer", "weight": 1.8},
    {"query": "What is the difference between Ready to Leave and Leaving Now hosting modes?", "type": "comparison", "expected_behavior": "answer", "weight": 1.8},
    
    # === PROCESS/FLOW (How things work) ===
    {"query": "How does the points redemption system work for beta users?", "type": "summarization", "expected_behavior": "answer", "weight": 1.5},
    {"query": "What approval workflow is used for admin configuration changes?", "type": "summarization", "expected_behavior": "answer", "weight": 1.5},
    {"query": "How long does Ready to Leave hosting mode stay active?", "type": "extraction", "expected_behavior": "answer", "weight": 1.0},
    
    # === EDGE CASES (Specific scenarios) ===
    {"query": "What is the maximum number of rematches allowed after a no-show?", "type": "numeric", "expected_behavior": "answer", "weight": 1.5},
    {"query": "Do credits expire if a user is inactive?", "type": "reasoning", "expected_behavior": "answer", "weight": 2.0},
    {"query": "What is the cash value of 10 reward points?", "type": "numeric", "expected_behavior": "answer", "weight": 1.5},
    
    # === HALLUCINATION TESTS (Should say "not found") ===
    {"query": "What is the weather forecast for tomorrow?", "type": "hallucination_test", "expected_behavior": "not_found", "weight": 3.0},
    {"query": "What is the stock price of Apple today?", "type": "hallucination_test", "expected_behavior": "not_found", "weight": 3.0},
    {"query": "Who is the CEO of GetSpace?", "type": "hallucination_test", "expected_behavior": "not_found", "weight": 3.0},
]
...
def parse_args():
    ...
    parser.add_argument(
        "--skip-warmup",
        action="store_true",
        help="Skip warm-up run (not recommended)"
    )
...
def score_response(response: str, expected_behavior: str, context: str, query_type: str) -> dict:
    ...
    if expected_behavior == "not_found":
        ...
        common_words = {"the", "and", "that", "this", "with", "from", "have", "been", "would", "could", "should", "about", "which", "there", "their", "what", "when", "where", "will", "were", "they", "your", "also", "into", "more", "some", "than", "them", "then", "these", "only", "other", "such", "after", "most", "make", "being", "well", "back", "much", "before"}
        response_words = set(w for w in response_lower.split() if len(w) > 4 and w not in common_words)
    ...
    conciseness = min(1.0, 50 / max(response_length, 1)) if query_type == "extraction" else min(1.0, 100 / max(response_length, 1))
    ...
    return {
        ...
        "grounding_ratio": round(grounding_ratio, 2) if expected_behavior != "not_found" else None,
...
def benchmark_model(model: str, prompt: str, query_info: dict, context: str) -> dict:
    ...
    try:
        ...
        # Score the response
        quality_scores = score_response(response_text, expected_behavior, context, query_type)
...
def print_query_summary(results: list, query: str):
    ...
    print("="*80)
    print(f"{'Model':<20} {'First Token':>12} {'Total Time':>12} {'Tok/s':>8} {'Quality':>12}")
    print("-"*80)
...
def print_overall_summary(results: list):
    ...
    print(f"{'Model':<20} {'Avg 1st Tok':>12} {'Avg Total':>12} {'Tok/s':>8} {'Halluc':>8} {'Quality':>10}")
    print("-"*80)
    ...
    for model, data in sorted(models.items()):
        ...
        quality_pct = (data['total_queries'] - data['hallucinations'] - data['instruction_failures']) / data['total_queries'] * 100
        
        print(f"{model:<20} {avg_first:>11.2f}s {avg_total:>11.2f}s {avg_toks:>8.2f} {halluc_rate:>8} {quality_pct:>9.0f}%")
    ...
    if baseline and baseline in models:
        baseline_first = sum(models[baseline]["first_tokens"]) / len(models[baseline]["first_tokens"])
        baseline_quality = (models[baseline]['total_queries'] - models[baseline]['hallucinations'] - models[baseline]['instruction_failures']) / models[baseline]['total_queries'] * 100
        ...
        for model, data in models.items():
            ...
            speed_score = max(0, min(100, (60 - avg_first) / 60 * 100))  # 0-60s maps to 100-0
            ...
            print(f"\n   {model}:")
            print(f"      Speed: {avg_first:.1f}s ({speed_improvement:+.0f}% vs baseline)")
            print(f"      Quality: {quality_pct:.0f}% (baseline: {baseline_quality:.0f}%)")
...
def save_results(results: list, output_path: str):
    ...
    for r in results:
        if r["status"] == "success":
            model = r["model"]
            if model not in models:
                models[model] = {"first_tokens": [], "total_times": [], "tok_per_secs": []}
            models[model]["first_tokens"].append(r["first_token"])
            models[model]["total_times"].append(r["total_time"])
            models[model]["tok_per_secs"].append(r["tok_per_sec"])
    ...
    for model, data in models.items():
        output["summary"][model] = {
            "avg_first_token": round(sum(data["first_tokens"]) / len(data["first_tokens"]), 2),
            "avg_total_time": round(sum(data["total_times"]) / len(data["total_times"]), 2),
            "avg_tok_per_sec": round(sum(data["tok_per_secs"]) / len(data["tok_per_secs"]), 2),
        }
...
def main():
    ...
    for query_info in queries:
        ...
        print(f"\n{'='*80}")
        print(f"QUERY: {query}")
        print(f"Type: {query_info['type']} | Expected: {query_info['expected_behavior']}")
        print('='*80)
