...
$COPILOT_EXCLUDE_PATTERN = "$GITHUB_DIR/$COPILOT_FILE_NAME"
...
# Sync copilot-instructions.md to target repo
function Sync-CopilotInstructions {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$SourceFile,
    ...
    # Always ensure git protection (even if sync was skipped)
    Ensure-GitProtection -RepoPath $TargetRepo -FilePattern $COPILOT_EXCLUDE_PATTERN -FilePath "$GITHUB_DIR\$COPILOT_FILE_NAME" -Results $Results
    
    return $true
}

# Sync sync_copilot_instructions.ps1 script to target repo
function Sync-ScriptFile {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$ScriptPath,
    ...
    $shouldUpdateScript = Compare-CopilotVersion -SourceVersion $ScriptVersion -TargetVersion $targetScriptVersion
    ...
    # Always ensure git protection for script
    ...
    Add-GitExclusion -RepoPath $TargetRepo -Pattern $SCRIPT_EXCLUDE_PATTERN | Out-Null
}
...
if (Test-GitTracked -RepoPath $sourceRepoRoot -File $COPILOT_EXCLUDE_PATTERN) {
    Write-Info -Message "Source file is tracked in git, removing from index..."
    if (Remove-FromGitIndex -RepoPath $sourceRepoRoot -File $COPILOT_EXCLUDE_PATTERN) {
...
if (Test-GitTracked -RepoPath $sourceRepoRoot -File $SCRIPT_EXCLUDE_PATTERN) {
    Write-Info -Message "Source script is tracked in git, removing from index..."
    if (Remove-FromGitIndex -RepoPath $sourceRepoRoot -File $SCRIPT_EXCLUDE_PATTERN) {
...
if (-not (Test-Path (Join-Path $sourceRepoRoot ".git"))) {
    ...
    Add-GitExclusion -RepoPath $sourceRepoRoot -Pattern $COPILOT_EXCLUDE_PATTERN | Out-Null
    Add-GitExclusion -RepoPath $sourceRepoRoot -Pattern $SCRIPT_EXCLUDE_PATTERN | Out-Null
}
...
foreach ($targetRepo in $TARGET_REPOS) {
    ...
    # Check if source and target are the same (skip self-sync)
    ...
    # Sync copilot-instructions.md
    Sync-CopilotInstructions -SourceFile $sourceFile -SourceHash $sourceHash -SourceVersion $sourceVersion -TargetRepo $targetRepo -Results ([ref]$results)
    
    # Sync the script itself
    Sync-ScriptFile -ScriptPath $scriptPath -ScriptHash $scriptHash -ScriptVersion $SCRIPT_VERSION -TargetRepo $targetRepo -Results ([ref]$results)
}
